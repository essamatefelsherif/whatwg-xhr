...
const recorded_xhr_events = [];
const actual_xhr_events = [];

const record_xhr_event = (e) => {
	const prefix = e.target.constructor.name === 'XMLHttpRequestUpload' ? 'upload.' : '';
	const event = (prefix || '') + e.type + '(' + e.loaded + ',' + e.total + ',' + e.lengthComputable + ')';

	recorded_xhr_events.push(event);
	actual_xhr_events.push(event);
}

const getNextEvent = (arr) => {
	let event = {
		str: arr.shift()
	};
	...
	let type = event.type = event.str.split('(')[0].split('.').pop();
	let loadedAndTotal = event.str.match(/.*\((\d+),(\d+),(true|false)\)/);
...
export function prepare_xhr_for_event_order_test(xhr){

	xhr.addEventListener('readystatechange', () => {
		recorded_xhr_events.push(xhr.readyState);
		actual_xhr_events.push(xhr.readyState);
	});

	const events = ['loadstart', 'progress', 'abort', 'timeout', 'error', 'load', 'loadend'];
...
export function assert_xhr_event_order_matches(expected){
	let recorded = recorded_xhr_events;
	let lastRecordedLoaded = -1;

	while(expected.length && recorded.length){
		let currentExpected  = getNextEvent(expected);
		let currentRecorded  = getNextEvent(recorded);
		...
		while(recorded.length && currentRecorded.type == `progress` && parseInt(recorded) === 3){

			assert(typeof currentRecorded.loaded === 'number');
			assert(currentRecorded.loaded > lastRecordedLoaded, `progress event 'loaded' values must only increase`);

			lastRecordedLoaded = currentRecorded.loaded;
		}

		if(currentRecorded.type == 'loadend'){
			let recordedProgressCount = 0;
			lastRecordedLoaded = -1;
		}

		assert(currentRecorded.str === currentExpected.str, `currentRecorded.str: ${currentRecorded.str} !== currentExpected.str: ${currentExpected.str}`);
	}
