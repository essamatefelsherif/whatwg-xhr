...
export default (activeURL) => {
	...
	(() => {
		...
		xhr.onload = (e) => {
			assert.strictEqual(xhr.responseText, 'æøå\nテスト\n');
		};

		xhr.open('GET', `${activeURL}/utf16.txt`);
		xhr.send();
	...
	(() => {
		...
		xhr.open('GET', `${activeURL}/status.py?code=200&type=text%2Fplain%3Bcharset%3DUTF-16&content=%E6%00%F8%00%E5%00%0A%00%C6%30%B9%30%C8%30%0A%00`);
		xhr.send();
	...
	(() => {
		...
		xhr.open('GET', `${activeURL}/status.py?code=200&type=text%2Fplain%3Bcharset%3DUTF-8&content=%E6%00%F8%00%E5%00%0A%00%C6%30%B9%30%C8%30%0A%00`);
		xhr.send();
