...
*/
	...
	function request(inputGenerator, headersToSend, expectedType, title){
		...
		xhr.open('POST', `${activeURL}/inspect-headers.py?filter_name=Content-Type`, false);

		for(let header in headersToSend){
			if(headersToSend.hasOwnProperty(header)){
				xhr.setRequestHeader(header, headersToSend[header]);
			}
		}

		xhr.send(toSend);

		const actual = xhr.responseText;
