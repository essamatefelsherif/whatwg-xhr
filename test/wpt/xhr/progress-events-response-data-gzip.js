...
export default (activeURL) => {
	...
	xhr.addEventListener('loadend', (e) => {
		...
		if(e.lengthComputable){
 			assert.strictEqual(e.total,  len, 'event.total is content-length');
			assert.strictEqual(e.loaded, len, 'event.loaded should be content-length at loadend');
		}
		else{
			assert.strictEqual(e.total, 0, 'if implementation can\'t compute event.total for gzipped content it is 0');
			assert(e.loaded >= len, 'event.loaded should be set even if total is not computable');
		}

		xhr.onloadend = xhr.progress = null;
	});
	...
	xhr.addEventListener('progress', (e) => {
		...
		if(e.lengthComputable && e.total && e.loaded){
			assert.strictEqual(e.total, lastTotal, 'event.total should remain invariant');
			assert(e.loaded <= lastTotal, 'event.loaded should not exceed content-length');
		}
		else{
			assert.strictEqual(e.total, 0, 'event.total should be 0');
		}

		assert(e.loaded > lastLoaded, 'event.loaded should only ever increase');
		lastLoaded = e.loaded;
	});

	xhr.open('GET', `${activeURL}/image.gif?pipe=gzip|trickle(45000:d1:r2)`);
	xhr.send();
}
