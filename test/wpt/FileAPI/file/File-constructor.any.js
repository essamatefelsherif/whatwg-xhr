...
export default (activeURL) => {
	...
	/* Test the existance of of File interface */
	(function test(){
		assert('File' in globalThis, 'globalThis should have a File property.');
	...
	(function test(){
		assert.throws(
			...
			(err) => {
				assert(err instanceof TypeError, 'Bits argument is required');
				return true;
			}
		);
		...
		assert.throws(
			...
			(err) => {
				assert(err instanceof TypeError, 'Name argument is required');
				return true;
			}
		);
	...
	['hello', 0, null].forEach(arg => {

		assert.throws(
			() => {
				new File(arg, 'world.html');
			},
			(err) => {
				assert(err instanceof TypeError, 'Constructor should throw for invalid bits argument');
				return true;
			}
		);
	});
	...
	(function test(){
		assert.throws(
			...
			(err) => {
				assert(err instanceof Error, 'Constructor should propagate exceptions');
				return true;
			}
		);
	...
	function test_first_argument(arg1, expectedSize, testName){
		...
		const file = new File(arg1, 'dummy');

		assert(file instanceof File);

		assert.strictEqual(file.name, 'dummy');
		...
		assert.notEqual(file.lastModified, '');
	}
	...
	[
		...
		const file = new File(['bits'], 'dummy', { type: testCase.type});

		assert(file instanceof File);
	    assert.strictEqual(file.type, testCase.expected);
	});

	(function test(){
		const file = new File(['bits'], 'dummy', { lastModified: 42 });

		assert(file instanceof File);
		assert.strictEqual(file.lastModified, 42);
	})('Using lastModified');

	(function test(){
		const file = new File(['bits'], 'dummy', { name: 'foo' });
	...
	(function test(){
		const file = new File(['bits'], 'dummy', { unknownKey: 'value' });
	...
	[
		...
		assert.throws(
			() => {
				new File(['bits'], 'name.txt', arg);
			},
			(err) => {
				assert(err instanceof TypeError, 'Constructor should throw for invalid property bag type');
				return true;
			}
		);
	});
	...
	[
		...
		assert.strictEqual(new File(['bits'], 'name.txt', arg).size, 4, 'Constructor should accept object-ish property bag type');
	});
