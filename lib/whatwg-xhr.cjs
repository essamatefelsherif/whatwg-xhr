...
/* Require local helper modules */
const {
	parse: contentTypeParse,
	format: contentTypeFormat
} = require('./helper/content-type.cjs');

const {
	dataURLProcessor,
	parseMIMEType,
	serializeAMimeType
} = require('./helper/whatwg-misc.cjs');

const {
	forbiddenRequestHeader,
	forbiddenHttpRequestMethod,
	safelyExtractBodyWithType
} = require('./helper/whatwg-fetch.cjs');

const {
	xmlEncodingSniffer
} = require('./helper/xml-encoding-sniffer.cjs');

const {
	File: FilePolyfill
} = require('./helper/file-polyfill.cjs');

/**
 * Global object added in node modules v20.0.0.
 *
 * @const {object} File
 * @desc  The File interface provides information about files and allows JavaScript in a web page to access their content.
 * @see   module:file-polyfill.File
 */
 ...
 */
const disableHeaderCheck = false;
...
class XMLHttpRequest extends XMLHttpRequestEventTarget{
	constructor(){
		...
		// request - An XMLHttpRequest object has an associated:
		...
		requestData['upload'] = new XMLHttpRequestUpload();    // xhr.spec. readonly attribute XMLHttpRequestUpload upload object:
		allowXMLHttpRequestUploadConstructor = false;          //           an XMLHttpRequestUpload object.
...
XMLHttpRequest.prototype.send = function send(body = null){
	...
	// @author. an implementation dependent method to inspect if any event listeners had been registered by calling addEventListener.
	// @todo.   suggest an implementation independent method
	...
	if(requestData['async']){
		...
		if(requestData['requestBody']){
			...
			(async () => {
				...
				if(requestData['requestBody'].source instanceof FormData){
					await new Promise((resolve, reject) => {
						requestData['requestBody'].stream.on('data', (chunk) => {
							buffer = Buffer.concat([buffer, Buffer.from(chunk)]);
						});

						requestData['requestBody'].stream.on('end', () => {
							resolve();
						});
					});
				}
...
const execString = `
	(async () => {
		...
		let { XMLHttpRequest } = await import('file:${filename}');
...
`;
		...
		if(bodyFlag && requestData['requestBody'].source instanceof Blob){
			...
			requestData['requestBody'].source = readBlobSync(requestData['requestBody'].source);
			syncProc.stdin.write(requestData['requestBody'].source);
		}
		else
		if(bodyFlag && requestData['requestBody'].source instanceof FormData){

			let buffer = Buffer.alloc(0);
			if(requestData['requestBody'].blobs){
				for(let part of requestData['requestBody'].blobs){
					if(part instanceof Blob){
						buffer = Buffer.concat([buffer, readBlobSync(part)]);
					}
					else
			...
			requestData['requestBody'].source = buffer;
			syncProc.stdin.write(requestData['requestBody'].source);
		}
		else
		if(bodyFlag){
...
class FormData{
	...
	constructor(form, submitter){
		...
		if(globalThis.HTMLFormElement){
			...
			if(form === null || typeof form !== 'object')
				throw new TypeError('FormData constructor: Argument 1 is not an object.');	// firefox 102

			if(!(form instanceof globalThis.HTMLFormElement))
				throw new TypeError(' FormData constructor: Argument 1 does not implement interface HTMLFormElement.');	// firefox 102
		...
		if(form !== undefined){
			throw new TypeError('FormData constructor: Argument 1 could not be converted to: undefined.');	// node.js
		}
	}
	...
	append(name, value, filename = undefined){
		...
		if(globalThis.HTMLFormElement){
			...
			if(arguments.length < 2)
				throw new TypeError(`FormData.append: ${arguments.length} is not a valid argument count for any overload.`);	// firefox 102
			else
			if(arguments.length === 3){
				if(typeof value !== 'object')
					throw new TypeError(`FormData.append: Argument 2 is not an object.`);	// firefox 102
				else
				if(!(value instanceof Blob))
					throw new TypeError(`FormData.append: Argument 2 does not implement interface Blob.`);	// firefox 102
			}
		...
		if(arguments.length === 0)
			throw new TypeError(`FormData.append: 2 arguments required, but 0 found.`);	// node.js
		else
		if(arguments.length === 1)
			throw new TypeError(`FormData.append: 2 arguments required, but only 1 found.`);	// node.js
		else
		if(arguments.length === 3 && !(value instanceof Blob))
			throw new TypeError(`Failed to execute 'append' on 'FormData': parameter 2 is not of type 'Blob'`);	// node.js

		name = String(name);

		value = value instanceof Blob ? value : String(value);
		filename = arguments.length === 3 ? String(filename) : undefined;
	...
	set(name, value, filename){
		...
		if(arguments.length === 0)
			throw new TypeError(`FormData.set: 2 arguments required, but 0 found.`); // node.js

		if(arguments.length === 1)
			throw new TypeError(`FormData.set: 2 arguments required, but only 1 found.`); // node.js

		if(arguments.length === 3 && !(value instanceof Blob))
			throw new TypeError(`Failed to execute 'set' on 'FormData': parameter 2 is not of type 'Blob'`); // node.js
	...
	entries(){
		...
		Object.defineProperty(iterator, Symbol.toStringTag, { value: 'FormData Iterator', writable: false, enumerable: false, configurable: true});

		const esIteratorPrototype = Object.getPrototypeOf(Object.getPrototypeOf([][Symbol.iterator]()));
		Object.setPrototypeOf(iterator, esIteratorPrototype);
	...
	keys(){
		...
		let iterator = {
			next(){
				return i < entryList.length ?
					{ value: Object.keys(entryList[i++])[0], done: false} :
					{ value: undefined, done: true};
			},
		};
		Object.defineProperty(iterator, Symbol.toStringTag, { value: 'FormData Iterator', writable: false, enumerable: false, configurable: true});
	...
	values(){
		...
		let iterator = {
			next(){
				return i < entryList.length ?
					{ value: Object.values(entryList[i++])[0], done: false} :
					{ value: undefined, done: true};
			},
		};
		Object.defineProperty(iterator, Symbol.toStringTag, { value: 'FormData Iterator', writable: false, enumerable: false, configurable: true});
...
/**
 * HTML Living Standard — Last Updated 7 January 2025
 * url: https://html.spec.whatwg.org/multipage/form-control-infrastructure.html#create-an-entry
 *
 ...
 *   Note:  These operations will create a new File object if either filename is given or the passed Blob is not a File object.
 ...
 * @return  {object} Return an entry whose name is name and whose value is value.
 * @desc    To create an entry given a string name, a string or Blob object value, and optionally a scalar value string filename.
 * @see     https://html.spec.whatwg.org/multipage/form-control-infrastructure.html#create-an-entry
 */
function makeEntry(name, value, filename) {

	// html.spec.1. Set name to the result of converting name into a scalar value string.
	name = Buffer.from(name).toString('utf8');

	// html.spec.2. If value is a string, then set value to the result of converting value into a scalar value string.
	if(typeof value === 'string'){
		value = Buffer.from(value).toString('utf8')
	}
	// html.spec.3. Otherwise:
	else {
		// html.spec.3.1. If value is not a File object, then set value to a new File object,
		//                representing the same bytes, whose name attribute value is "blob".
		if(!(value instanceof File)){
			value = new File([value], 'blob', { type: value.type });
		}

		// html.spec.3.2. If filename is given, then set value to a new File object,
	...
	// html.spec.4. Return an entry whose name is name and whose value is value.
	return { [name]: value };
}
 ...
 * @return  {Buffer} Return a buffer object with the contents of the Blob or File given.
 * @desc    Reads the contents of the Blob or File object synchronously, thus blocking the Node.js event loop and further JavaScript execution until return.
 */
function readBlobSync(blob){
	...
	let buffer = fs.readFileSync(contentBlob);
...
/* exports */
module.exports.disableHeaderCheck = disableHeaderCheck;
